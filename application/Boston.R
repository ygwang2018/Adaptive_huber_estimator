...
data <- Boston
y1 <- data$medv
x1 <- cbind(data$rm, data$age, data$dis, data$tax,
            data$ptratio, data$black, data$lstat)
...
for (b in 1:B) {
  ...
  # Step 2: 在 bootstrap 样本内部划分训练/测试（不去重：按位置切分）
  pos_train <- sample.int(n, size = 500, replace = FALSE)  # 选 500 个位置
  
  train_idx_in_boot <- boot_idx[pos_train]
  test_idx_in_boot  <- boot_idx[-pos_train]                # 剩余位置，重复保留
