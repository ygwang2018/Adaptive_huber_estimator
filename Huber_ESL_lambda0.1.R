...
for (tau_parmater in  tau_parmater_lis) {
  ...
  for (i in 1:1000) {
    ...
    Huber_ESL_beta <- Huber_ESL_IRLS(LMS_beta, tau = tau_parmater, lambda = 0.1,
                                     X = new_X, Y = Y,
                                     S_n = Sn, eps = 1e-3,
                                     max_iter = 500)
...
write.csv(df, "C:/Users/apple/Desktop/modified_huber/diff_tau_esl_huberlambda0.1.csv", row.names = FALSE)
