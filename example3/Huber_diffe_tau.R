...
diff_tau_huber_MSE_lis=c()
diff_tau_huber_bias_lis=c()



tau_parmater_lis=c(0.2,0.4,0.6,0.8,1.1,1.3,1.5,1.7,1.9,2.1,2.3,2.5,2.7)
for (tau_parmater in  tau_parmater_lis) {
  ...
  for (i in 1:1000) {
    ...
    Huber_beta <- Huber_IRLS(LMS_beta, tau = tau_parmater,
                             X = new_X, Y = Y,
                             S_n = Sn, eps = 1e-3,
                             max_iter = 500)
  ...
  print(mean(huber_MSE_lis))
  diff_tau_huber_MSE_lis=c(diff_tau_huber_MSE_lis,mean(huber_MSE_lis))
  diff_tau_huber_bias_lis=c(diff_tau_huber_bias_lis,mean(huber_bias_lis))
  
}

diff_tau_huber_MSE_lis
diff_tau_huber_bias_lis
df <- data.frame(
  tau = tau_parmater_lis,
  mse   = diff_tau_huber_MSE_lis,
  bias  = diff_tau_huber_bias_lis
)

write.csv(df, "C:/Users/apple/Desktop/modified_huber/diff_tau_huber.csv", row.names = FALSE)
