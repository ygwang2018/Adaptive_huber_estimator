...
diff_power_Mohuber_MSE_lis=c()
diff_power_Mohuber_bias_lis=c()


power_parmater_lis<-c(1.05,1.1,1.5,1.8,2,2.2,2.5,2.8,3,3.2,3.5,3.7,4)

for (power_parmater in  power_parmater_lis) {
  Mohuber_MSE_lis=c()
  Mohuber_bias_lis=c() 
  for (i in 1:1000) {
    set.seed(i)
    ...
    #c=9999 c_total=10000
    res <- abs_normal_q(c_seg = 49+1, total_node = 120, alpha =1, power = power_parmater)
    knot_vec <- res$q
  ...
  print(mean(Mohuber_MSE_lis))
  diff_power_Mohuber_MSE_lis=c(diff_power_Mohuber_MSE_lis,mean(Mohuber_MSE_lis))
  diff_power_Mohuber_bias_lis=c(diff_power_Mohuber_bias_lis,mean(Mohuber_bias_lis))
  
}
#optial_c<-20000 0.999
diff_power_Mohuber_MSE_lis
diff_power_Mohuber_bias_lis
df <- data.frame(
  power = power_parmater_lis,
  mse   = diff_power_Mohuber_MSE_lis,
  bias  = diff_power_Mohuber_bias_lis
)

write.csv(df, "C:/Users/PC/Desktop/modified_huber/example3/diff_power_results_totalnode120.csv", row.names = FALSE)
