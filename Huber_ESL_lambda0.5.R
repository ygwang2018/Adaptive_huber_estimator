...
diff_tau_huber_esl_MSE_lis=c()
diff_tau_huber_esl_bias_lis=c()



tau_parmater_lis=c(1.1,1.3,1.5,1.7,1.9,2.1,2.3,2.5,2.7,2.9,3.1,3.3,3.5)
for (tau_parmater in  tau_parmater_lis) {
  huber_esl_MSE_lis=c()
  huber_esl_bias_lis=c() 
  for (i in 1:1000) {
    set.seed(i)
    ...
    Huber_ESL_beta <- Huber_ESL_IRLS(LMS_beta, tau = tau_parmater, lambda = 0.5,
                                     X = new_X, Y = Y,
                                     S_n = Sn, eps = 1e-3,
                                     max_iter = 500)
  ...
  print(mean(huber_esl_MSE_lis))
  diff_tau_huber_esl_MSE_lis=c(diff_tau_huber_esl_MSE_lis,mean(huber_esl_MSE_lis))
  diff_tau_huber_esl_bias_lis=c(diff_tau_huber_esl_bias_lis,mean(huber_esl_bias_lis))
  
}

diff_tau_huber_esl_MSE_lis
diff_tau_huber_esl_bias_lis
df <- data.frame(
  tau = tau_parmater_lis,
  mse   = diff_tau_huber_esl_MSE_lis,
  bias  = diff_tau_huber_esl_bias_lis
)

write.csv(df, "C:/Users/apple/Desktop/modified_huber/diff_tau_esl_huberlambda0.5.csv", row.names = FALSE)
